...
import { transactions, insertTransactionsSchema, categories, accounts } from '../../../../database/schema';
        ...
        async c => {
            ...
            const transactionsToDelete = db.$with('transactions_to_delete').as(
                db.select({ id: transactions.id })
                  ...
                  .innerJoin(accounts, eq(transactions.accountId, accounts.id))
                  .where(
                     and(inArray(transactions.id, values.ids), eq(accounts.userId, auth.userId)),
                  ),
            );
            const data = await db
